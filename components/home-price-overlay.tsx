...
/** Defaults for desktop card DBUG (localStorage overrides). */
const DESKTOP_CARD_DEFAULT: LayoutMap = {
  weekly: {
...
const DESKTOP_CARD_STORAGE = "foam-pricing-line-layout-desktop-v1";
const DESKTOP_CARD_DBUG = "foam-pricing-dbug-desktop-cards";
const DESKTOP_CARD_LINK = "foam-pricing-dbug-link-desktop-cards";
...
function loadDesktopCardLayout(): LayoutMap {
  try {
    const raw = localStorage.getItem(DESKTOP_CARD_STORAGE);
    if (!raw) return structuredClone(DESKTOP_CARD_DEFAULT);
    const parsed = JSON.parse(raw) as Partial<LayoutMap>;
    const next = structuredClone(DESKTOP_CARD_DEFAULT);
    ...
    return structuredClone(DESKTOP_CARD_DEFAULT);
  }
}

function useDesktopCardDbug() {
  const [enabled, setEnabled] = useState(true);
  const [linked, setLinkedState] = useState(false);
  const [layout, setLayout] = useState<LayoutMap>(DESKTOP_CARD_DEFAULT);
  const [copied, setCopied] = useState(false);
  const linkedRef = useRef(false);

  useEffect(() => {
    localStorage.setItem(DESKTOP_CARD_DBUG, "1");
    setEnabled(true);
    const linkSaved = localStorage.getItem(DESKTOP_CARD_LINK) === "1";
    setLinkedState(linkSaved);
    linkedRef.current = linkSaved;
    setLayout(loadDesktopCardLayout());
  }, []);
  ...
  const patchLine = (
    ...
    setLayout((prev) => {
      ...
      localStorage.setItem(DESKTOP_CARD_STORAGE, JSON.stringify(next));
      return next;
    });
  };
  ...
  const setDbug = (on: boolean) => {
    setEnabled(on);
    localStorage.setItem(DESKTOP_CARD_DBUG, on ? "1" : "0");
  };

  const setLinked = (on: boolean) => {
    setLinkedState(on);
    linkedRef.current = on;
    localStorage.setItem(DESKTOP_CARD_LINK, on ? "1" : "0");
  };

  const reset = () => {
    const fresh = structuredClone(DESKTOP_CARD_DEFAULT);
    setLayout(fresh);
    localStorage.setItem(DESKTOP_CARD_STORAGE, JSON.stringify(fresh));
  };
